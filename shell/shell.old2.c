...
void split_line_at_whitespaces(char *s_string, char **a_arguments, char **a_variables)
{
    ...
    while (s_part != NULL)
    {
        ...
        if (n_index_of_equal_symbol)
        {
            ...
            // char tmp_str[20];
            // strcpy(tmp_str,  s_part);
            // strcpy(tmp_str,  (char[2]) { (char) 65+n_a_variables_length, '\0' });
            // dnds(tmp_str);
            // strcpy(a_variables[n_a_variables_length], s_part);
            // a_variables[n_a_variables_length] = tmp_str;
            // a_variables[n_a_variables_length] = (char *) 65 + n_a_variables_length;
            // a_variables[n_a_variables_length] = s_time;
