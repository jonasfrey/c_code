...
#include <stdio.h>
#include <string.h>
#include <libusb-1.0/libusb.h> //sudo apt-get install libusb-1.0-0-dev
...
int main(int argc, char *argv[])
{
	...
	if (device_name) {
		...
		for (i = 0; devs[i]; i++){
			...
			// // 		ret = libusb_get_string_descriptor_ascii(handle, desc.iManufacturer, string, sizeof(string));
			// // 		if (ret > 0)
			// // 			printf("\nvendorid:%s", (char *)string);
			// // 	}
			// // 	if (desc.iProduct) {
			// // 		ret = libusb_get_string_descriptor_ascii(handle, desc.iProduct, string, sizeof(string));
			// // 		if (ret > 0)
			// // 			printf("\nproductid:%s", (char *)string);
			// // 	}
			// // }
			// printf("\nDEVICE");
			...
			if (handle) {
				printf("\nvendorid:productid -> %0+4x:%0+4x", desc.idVendor,desc.idProduct);
